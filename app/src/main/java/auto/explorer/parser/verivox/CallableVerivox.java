...
import static auto.explorer.parser.verivox.MapsVerivox.carTypeCodes;
import static auto.explorer.parser.verivox.MapsVerivox.damagedCarsCodes;
import static auto.explorer.parser.verivox.MapsVerivox.extColorCodes;
import static auto.explorer.parser.verivox.MapsVerivox.fuelTypeCodes;
import static auto.explorer.parser.verivox.MapsVerivox.gearTypeCodes;
import static auto.explorer.parser.verivox.MapsVerivox.interiorEquipCodes;
import static auto.explorer.parser.verivox.MapsVerivox.landCodes;
import static auto.explorer.parser.verivox.MapsVerivox.providerCodes;
import static auto.explorer.parser.verivox.MapsVerivox.sortTypeCodes;
...
public final class CallableVerivox implements Callable<ArrayList<Car>> {

    public static final String URL = "http://www.verivox.de/auto/suche/";
    ...
    public CallableVerivox(@NonNull SearchProfile profile, String page, RequestParserHelper.TaskCompleteListener listener) {
        this.p = profile;
        this.page = page;
    ...
    public ArrayList<Car> call() throws Exception {
        ...
        addParam("atype", "C");
        final String markCode = MapsVerivox.marksCarCodes.get(p.getMark());
        if (markCode != null && !markCode.equals(Const.ALL)) {
            addParam("mmvmk0", markCode);
            if (!p.getModel().equals(ALL)) {
                final String modelCode = MapsVerivox.getModelCode(p.getMark(), p.getModel());
        ...
        try {
            ...
            cars = DocParserVerivox.parseDoc(document);
        } catch (IOException e) {
            e.printStackTrace();
        }
