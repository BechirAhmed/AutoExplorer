...
import static auto.explorer.parser.mobile.de.MapsMobileDe.carTypeCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.damagedCarsCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.extColorCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.fuelTypeCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.gearTypeCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.interiorEquipCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.landCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.providerCodes;
import static auto.explorer.parser.mobile.de.MapsMobileDe.sortTypeCodes;
...
public final class CallableMobileDe implements Callable<ArrayList<Car>> {

    private static final String URL = "http://suchen.mobile.de/fahrzeuge/search.html";
    ...
    public CallableMobileDe(@NonNull SearchProfile profile, String page, RequestParserHelper.TaskCompleteListener listener) {
        this.p = profile;
        this.page = page;
    ...
    public ArrayList<Car> call() throws Exception {
        ...
        final String markCode = MapsMobileDe.marksCarCodes.get(p.getMark());
        if (markCode != null && !markCode.equals(Const.ALL)) {
            addParam("makeModelVariant1.makeId", markCode);
            if (!p.getModel().equals(ALL)) {
                final String modelCode = MapsMobileDe.getModelCode(p.getMark(), p.getModel());
                if (modelCode != null && !modelCode.equals(Const.ALL))
                    addParam("makeModelVariant1.modelId", modelCode);
        ...
        try {
            ...
            cars = DocParserMobileDe.parseDoc(document);
        } catch (IOException e) {
            e.printStackTrace();
        }
